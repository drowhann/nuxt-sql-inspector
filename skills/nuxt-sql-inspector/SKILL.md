...
`sqlInspector.enabled` (default: `nuxt.options.dev`), `forceEnableInProduction` (required with `enabled` outside dev), `path`, `apiBase`, `maxRequests` (default 200, max 1000), optional `include` / `exclude` path globs.
