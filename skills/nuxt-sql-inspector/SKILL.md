...
description: >-
  ...
  inspection for pg, postgres.js, mysql2, @libsql/client, better-sqlite3, or db0 / Nitro
  useDatabase() (Neon Pool, Vercel/Netlify/PGlite, Drizzle, Prisma adapter), or wiring /__sql_queries.
---

# nuxt-sql-inspector

Dev-only Nuxt module (Nuxt 3 and Nuxt 4): API requests + SQL queries at `/__sql_queries`. Wraps drivers (`pg` / `postgres.js` / `mysql2` / `@libsql/client` / `better-sqlite3` / db0), not a specific ORM.
...
Postgres-compatible `.query` clients use `/node-postgres` (Neon Pool, Vercel createPool, Netlify `db.pool`, PGlite). Neon HTTP `neon()`, Bun `SQL`, TiDB serverless, `bun:sqlite` are **not** supported. Full list: project `docs/usage.md` Compatibility section.

Wrapping a pg Pool also records `pool.connect()` / checked-out `client.query`. Wrapping a mysql2 pool also records `getConnection()`. Nitro `useDatabase()` → `/db0` (do not also wrap the inner better-sqlite3 instance). Do not auto-patch. Call once at pool/client creation. When the module is disabled (production), the same imports resolve to a no-op `inspectSql`.
